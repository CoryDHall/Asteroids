(function() {
  ...
  MovingObject.prototype.draw = function (ctx) {
    ...
    ctx.lineWidth = 2 - 1.5 * (this.capStyle === "square");
    ctx.arc(
      this.pos[0] - i * dimX,
      this.pos[1] - j * dimY,
      this.radius,
    ...
    ctx.stroke()
    ctx.beginPath();
    ctx.lineWidth = 1 - 0.5 * (this.capStyle === "square");
    ctx.arc(
      this.pos[0] - i * dimX,
      this.pos[1] - j * dimY,
    ...
    // ctx.closePath();
    ctx.beginPath();
    ctx.lineWidth = 3 - 2.5 * (this.capStyle === "square");
    ctx.arc(
      this.pos[0] + v[0] - i * dimX,
      this.pos[1] + v[1] - j * dimY,
      this.radius - 4 + 3 * (this.capStyle === "square"),
  ...
  MovingObject.prototype._gravity = function () {
    ...
    this.game.asteroids.concat([this.game.ship]).forEach(function (otherObj) {
      ...
      force = -MovingObject.G * otherObj._mass / (Math.sqrt(dist[0] * dist[0] + dist[1] * dist[1]) || 1);
  ...
  MovingObject.prototype.move = function () {
    ...
    if (Math.abs(this.vel[0]) > Asteroids.Game.DIM_X / 2 || Math.abs(this.vel[1]) > Asteroids.Game.DIM_Y) {
