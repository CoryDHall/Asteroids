(function() {
  ...
  MovingObject.prototype._gravity = function () {
    ...
    this.game.asteroids.concat([this.game.ship]).forEach(function (otherObj) {
      ...
      var force = -gConst * (otherObj.mass() * this.mass()) / Math.sqrt(dist[0] * dist[0] + dist[1] * dist[1] + this.radius);

      this.accel[0] += (dist[0] * force) / this.mass();
      this.accel[1] += (dist[1] * force) / this.mass();
