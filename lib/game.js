(function () {
    ...
    setInterval(function () {
      if (this.addingAsteroids) {
        if (Math.random() < (1 / Asteroids.Asteroid.RADIUS)) {
          ...
          bigAsteroid.radius *= 40 + (60 * Math.random());
  ...
  Game.prototype.draw = function(ctx, $overlay) {
    ...
    $overlay.text("Mass: " + this.ship.mass().toExponential());
    $overlay.append("<br>Asteroids Left: " + this.asteroids.length);
    this.asteroids.forEach(function (asteroid) {
      var $asteroidInfo = $("<p>")
        .css({ color: asteroid.getColor() })
        .text(asteroid.density.toExponential(4) + "::" + asteroid.vel.map(function (coord) { return coord.toExponential(4) }).toString());
