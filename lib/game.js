(function () {
  ...
  Game.prototype.tempFillColor = function (newColor) {
    ...
    var transition = setInterval(function () {
      ...
      console.log(light, this._fillColor);
    }.bind(this), 50);
    setTimeout(function () {
  ...
  Game.prototype.checkCollisions = function() {
    ...
    objects.forEach(function (object1, idx) {
      ...
      var collided = objects.slice(idx + 1).every(function (object2) {
  ...
  Game.prototype.step = function () {
    if (this.gTime - this.oldTime > 1000 / 24 || this._diag.pause) {
      this.oldTime = this.gTime;
      return;
    }
    this.checkCollisions();
    ...
    if (!this.addingAsteroids && this.asteroids.length <= Math.ceil(Game.NUM_ASTEROIDS / 3)) {
      ...
      bigAsteroid.density *= 1000.0;
      bigAsteroid.radius = 300;
